...
public class CSizeUtils {
	...
	private static void fixedSize(Component c, boolean min, boolean max) {
		...
		if (min && max) {
			...
			if (min) fixedMinSize(c);
			if (max) fixedMaxSize(c);
		}
	}
	 ...
	 * Add this to the preferred height of the scroller in case a horizontal scroll-bar is always shown.
	 */
	 ...
	 * Add this to the preferred width of the scroller in case a vertical scroll-bar is always shown.
	 */
