 ...
 * this class shares code for both classes.
 * However, there are subtle differences between the methods 
 * and sharing to much might make debugging difficult.
 * One example of a method that is difficult to share is <code>maximumLayoutSize</code>
 * @author fred
 *
 */
public abstract class HVLayout implements LayoutManager2 { 
	...
	public HVLayout(HVSize props) {
		super();
		if (props == null) props = HVSize.getDefault();
		setHvprops(props);
	}
	...
	/** Adds a component at the end of the list or 
	 * if place is a valid index (Integer class required)
	 * adds the component at the specified
	 * index-place (0 is the first place). 
	...
	/**
	 * This method ensures that temporary values can be used.
	 * Only this function can give the signal that a re-calculation has to be done
...
//System.out.println("Invalidate HVLayout");		
	...
	/**
	 * The total gap width, used in resizing calculations.
	 * Note that this is a slightly different calculation then
	 * gapHeight() in VLayout. 
	 */
	...
	public Dimension minimumLayoutSize(Container target, boolean vert) {
		...
		if (dmin != null)
			// By not returning dmin itself we ensure that dmin values cannot be changed from the outside
