...
/**
 * A layout-manager that aligns components vertically and shrinks or
 * grows components vertically as specified by their minimum, preferred and maximum sizes.
 * Use {@code VBox.add(Component)} to put each added component 
 * beneath the previous one and let this layout manager do the vertical resizing.
 * <p>
 * The component's sizes must be set properly for this layout manager 
 * to work. Use {@code HVProps.setSize()} to accomplish this.
 * Use {@code HVProps} many default values to change to appearance of the screen. 
 * <p> 
 * Components shrink vertically to their minimum size when their preferred size 
 * is larger then their minimum size.<br>
 * Likewise, components will grow vertically when their maximum size is greater
 * then their preferred size.<br>
 * Also, components shrink and grow relative to how much other components want to
 * shrink or grow. For example, if one text area (A) needs to be twice as big as 
 * another text area (B), then simply setting the preferred height of B to 
 * twice the preferred height of A will do the trick. See for example 
 * {@code TestUI} which calls 
 * <br> {@code HVProps.getScroller(text, 0, 3, false)} and 
 * <br> (@code HVProps.getScroller(text, 0, 6, false)}
 * to show this effect. 
 * <p>
 * This class determines the size for components vertically. The {@code HLayout} 
 * class determines the size for components horizontally and
 * can be used together with HBox to order components in
 * the reading direction, from the middle or opposite the reading direction.
 * <p>
 * Other layour managers should have no trouble with this layout manager. 
 * In other words, {@code VBox and HBox} can be part of a component that uses  
 * BorderLayout or Flowlayout, etc.
 * <p>
 * Add instances of {@code VBox} (which uses {@code VLayout}) and {@code HBox} (which uses
 * {@code HLayout}) to the content pane of frames and dialogs (and/or make
 * a {@code VBox} with insets the content pane).
 * <p>
 * Developers note:
 * VLayout and HLayout must be kept in sync to work.  
 */
public class VLayout extends HVLayout { 
	...
	/** Set the general alignment to a value that corresponds to
	the line axis orientation, i.e.
	...
	*/
	...
	/** 
	 * Return minimum size (width and height) of this container
	...
	/** 
	 * Return the maximum size (width and height) of this container
	...
	/** 
	Determines size and position of all components/lines
	according to the available size given via the target parameter.
	Minimum and maximum size of components will be respected 
	so they may be non-visible if the target is too small 
	(component falls outside of Window).
	The frame.pack() funtion usually prevents this. */
	@Override
